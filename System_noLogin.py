...
from EmotionDetModel import EmotionDetector, count_classes
...
class RecMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None, *args, **kwargs):
        ...
        QWindowCtrls(self, exit_title="人脸表情识别系统",  # 退出确认框标题
        ...
        self.count_name = ["愤怒", "厌恶", "恐惧", "快乐", "自然", "悲伤", "惊讶"]  # 定义类名列表
        ...
        # 加载模型
        self.model = EmotionDetector()  # 模型实例
        # 加载训练的模型权重
        self.model.load_models((abs_path("models/deploy.prototxt", path_type="current"),
                                abs_path("models/res10_300x300_ssd_iter_140000.caffemodel", path_type="current")),
                               abs_path("models/best_mobilenet_model_f1_65.pt", path_type="current"))

        self.plot_verticalBar(self.label_bar, self.count_name, [0 for i in self.count_name],
                              self.colors, margin=10)
    ...
    def toggle_comboBox(self):
        ...
        if len(self.detInfo) > 0:  # 检查检测结果列表是否为空
            ...
            self.label_score_result.setText('%.2f' % (self.detInfo[ind_select]["emotion_confidence"]))  # 显示置信度值
            # 显示位置坐标
            ...
            for i, det in enumerate(self.detInfo):  # 遍历所有标记框
                if ind != -1:
                    if ind != i:
                        continue
                # 在图像上标记目标框
                name, bbox, conf, cls_id = det['class_name'], det['bbox'], det["emotion_confidence"], det['class_id']
                ...
                emotion_scores = self.detConf[i]
                scaled_scores = [score * 100 for score in list(emotion_scores.values())]
                self.plot_verticalBar(self.label_bar, self.count_name, scaled_scores, self.colors, margin=10)
            # 在Qt界面中显示检测完成画面
    ...
    def frame_process(self, image):
        ...
        t1 = time.time()
        pred, emotion_scores = self.model.predict(pre_img)  # 使用模型进行预测
        t2 = time.time()
        ...
        if det is not None and len(det):
            ...
            self.detConf = emotion_scores.copy()
            ...
            if len(det_info):
                ...
                name, bbox, conf, cls_id = det_info[0]['class_name'], det_info[0]['bbox'], \
                    det_info[0]["emotion_confidence"], det_info[0]['class_id']
                ...
                self.comboBox_select.addItem('所有目标')  # 添加"所有目标"选项
                scaled_scores = [score * 100 for score in list(emotion_scores[0].values())]
                self.plot_verticalBar(self.label_bar, self.count_name, scaled_scores, self.colors, margin=10)
                ...
                for info in det_info:  # 遍历检测信息
                    name, bbox, conf, cls_id = info['class_name'], info['bbox'], info["emotion_confidence"], info['class_id']
    ...
    def toggle_version(self):
        ...
        QMessageBox.about(self.centralwidget,
                          "官网",
                          "<A href='http://zs.yjs.cqut.edu.cn/'>"
                          "http://zs.yjs.cqut.edu.cn/</a>")  # 在消息框中显示官网的链接
