...
from EmotionDetModel import EmotionDetector
...
if __name__ == '__main__':  # 如果当前模块是主模块
    model = EmotionDetector()  # 创建YOLOv8Detector对象
    model.load_models(("./models/deploy.prototxt",
                       "./models/res10_300x300_ssd_iter_140000.caffemodel"),
                      "./models/best_mobilenet_model.pt")
    colors = get_cls_color(model.names)  # 获取类别颜色

    app = QtWidgets.QApplication(sys.argv)  # 创建QApplication对象
    window = MainWindow()  # 创建MainWindow对象

    img_path = abs_path("test_media/happy2.png")  # 定义图像文件的路径
